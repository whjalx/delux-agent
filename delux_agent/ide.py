...
class DeluxIDE:
    def __init__(self, config: Config, cwd: Path, max_steps: int = 12) -> None:
        ...
        self._ask_mode = True
        self._loaded_session_context: str | None = None  # injected on next run
        self._session_context_turns = 2                  # how many past runs to include
        ...
        self._install_sidebar_redraw()

    def _setup_sigwinch_handler(self) -> None:
        """Handle terminal resize — redraw sidebar when terminal width changes."""
        import signal
    ...
    def _install_sidebar_redraw(self) -> None:
        """Wrap sys.stdout.write to auto-redraw sidebar after each output."""
        original_write = sys.stdout.write
        sb = self._sidebar

        def wrapped_write(text: str) -> int:
            result = original_write(text)
            # Redraw sidebar after newlines or flush
    ...
    def _refresh_sidebar(self) -> None:
        """Update sidebar state from current IDE state and redraw."""
        sb = self._sidebar
    ...
    def _get_prompt_toolkit_session(self):
        ...
        try:
            ...
            def _toggle_plan(event):
                """Toggle plan mode in-place. No new line, no session exit."""
            ...
            # Pywal integration / Purple Fallback
            wal_bg = '#1a1a2e'
            wal_fg = '#c8c4c3'
            wal_accent = '#bd93f9' # Dracula purple fallback
            wal_active = '#ffb86c' # Orange/Yellow active state
            wal_sep = '#6272a4'
            wal_label = '#44475a'
            ...
            try:
                ...
                if os.path.exists(wal_path):
                    ...
                    wal_accent = c.get('color5', wal_accent) # color5 is usually magenta/purple
                    wal_active = c.get('color3', wal_active) # color3 is usually yellow
                    wal_sep = c.get('color8', wal_sep)       # bright black / gray
                    wal_label = c.get('color7', wal_label)   # white / light gray
            except Exception:
                pass
    ...
    def _get_toolbar(self):
        """Dynamic bottom toolbar for prompt_toolkit. Returns FormattedText."""
    ...
    def _build_pt_prompt(self):
        """Dynamic prompt text for prompt_toolkit."""
        from prompt_toolkit.formatted_text import FormattedText
    ...
    def _show_shortcut_bar(self) -> None:
        """No-op when prompt_toolkit is active (toolbar handles it). Fallback for non-PTK."""
        if getattr(self, '_pt_session', None) is not None:
            return  # toolbar is always visible — nothing to print
    ...
    def _draw_prompt(self, buffer: str, cursor_pos: int | None = None) -> None:
        """Legacy fallback prompt draw — only used when prompt_toolkit is unavailable."""
    ...
    def _read_input_raw(self) -> str | None:
        """Read a line of input using prompt_toolkit (preferred) or raw fallback."""
        ...
        # ── Fallback (non-TTY or no prompt_toolkit)
        try:
            return input(f"{CYAN}{BOLD}{self._i18n.t('prompt.label')}{RESET} {DIM}❯{RESET} ").strip()
        except EOFError:
            return None
        except KeyboardInterrupt:
            return ''
    ...
    def run(self) -> int:
        ...
        self._get_prompt_toolkit_session()  # warm up session + toolbar
        while True:
            try:
            ...
            # Ctrl+L clear signal from prompt_toolkit keybinding
            if raw == '__CLEAR__':
    ...
    def _print_banner(self) -> None:
        ...
        model_name = m.name
        endpoint = m.api_endpoint or (self._active_api_base().rstrip("/") + "/chat/completions")
        home_str = str(self.config.root)

        flags = []
        lang = (self.config.lang or "en").upper()
        flags.append(lang)
        flags.append(f"validate:{self._validate_mode}")
        if self._plan_mode:
            flags.append(f"{YELLOW}PLAN{RESET}")
        if self._ask_mode:
            flags.append(f"{CYAN}ASK{RESET}")
        if self._ephemeral:
            flags.append(f"{MAGENTA}EPHEMERAL{RESET}")
        if self._ctx_cfg.enabled:
            flags.append(f"{GREEN}CTX{RESET}")
        ...
        if self.state.plan:
            p = self.state.plan
            plan_info = f"  {DIM}│{RESET} {YELLOW}Plan {p.progress()}{RESET}"

        # ── Inner width (inside the box = w - 4 for "  │" + "│  " padding)
        inner = w - 4

        def _row(content: str, raw_len: int | None = None) -> str:
            """Print a box row. raw_len = visual length of content (no ANSI)."""
            if raw_len is None:
                raw_len = len(re.sub(r'\x1b\[[0-9;]*m', '', content))
            pad = max(0, inner - raw_len - 1)
            return f"  {DIM}│{RESET} {content}{' ' * pad}{DIM}│{RESET}"

        # Title
        if self._plan_mode:
            title_raw = f"◆ DELUX  [PLANNING]"
            title_fmt = f"{BOLD}{MAGENTA}◆ DELUX{RESET}  {DIM}[{YELLOW}PLANNING{RESET}{DIM}]{RESET}"
        else:
            title_raw = f"◆ DELUX  Autonomous Shell Agent"
            title_fmt = f"{BOLD}{MAGENTA}◆ DELUX{RESET}  {DIM}Autonomous Shell Agent{RESET}"

        # Model line
        model_raw = f"{provider} · {model_name}"
        model_fmt = f"{CYAN}{provider}{RESET} {DIM}·{RESET} {model_name}"

        # Endpoint line (truncated)
        ep_display = endpoint[:inner - 2] if len(endpoint) > inner - 2 else endpoint
        ep_raw = ep_display
        ep_fmt = f"{DIM}{ep_display}{RESET}"

        # Home line
        home_display = home_str[:inner - 8] if len(home_str) > inner - 8 else home_str
        home_raw = f"home: {home_display}"
        home_fmt = f"{DIM}home: {RESET}{home_display}"

        # Flags line
        flags_raw_parts = [re.sub(r'\x1b\[[0-9;]*m', '', f) for f in flags]
        flags_raw = "  ".join(flags_raw_parts)
        flags_fmt = f"  ".join(flags) + plan_info

        # Skills line
        skills_line_raw = ""
        skills_line_fmt = ""
        if self._skill_names:
            preview = ", ".join(self._skill_names[:6])
            if len(self._skill_names) > 6:
                preview += f" +{len(self._skill_names) - 6} more"
            skills_line_raw = f"skills: {preview}"
            skills_line_fmt = f"{DIM}skills:{RESET} {CYAN}{preview}{RESET}"

        sep = f"  {DIM}├{'─' * (w - 4)}┤{RESET}"

        print()
        print(f"  {DIM}╭{'─' * (w - 4)}╮{RESET}")
        print(_row(title_fmt, len(title_raw)))
        print(sep)
        print(_row(model_fmt, len(model_raw)))
        print(_row(ep_fmt, len(ep_raw)))
        print(_row(home_fmt, len(home_raw)))
        if flags_raw.strip():
            print(_row(flags_fmt, len(flags_raw)))
        if skills_line_raw:
            print(_row(skills_line_fmt, len(skills_line_raw)))
        print(f"  {DIM}╰{'─' * (w - 4)}╯{RESET}")
        print()
    ...
    def _cmd_lang(self, args: list[str]) -> None:
        ...
        if lang in ("en", "es"):
            ...
            # Reset cached prompt_toolkit session so toolbar rebuilds with new language
    ...
    def _cmd_train(self, args: list[str]) -> None:
        ...
        if action == "on":
            ...
            if path.exists():
                print(f"  Size: {_estimate_file_size(path)}")
                print(f"  Examples: {_count_dataset_lines(path)}")
        else:
    ...
    def _build_progress_bar(self, progress: str) -> str:
        """Build a visual progress bar from '3/8' style string."""
        ...
        return f"{GREEN}[{bar}]{RESET} {DIM}{progress}{RESET}"
    ...
    def _show_plan_ui(self) -> None:
        ...
        print(f"\n{BOLD}{MAGENTA}\u25c6 PLAN{RESET} {DIM}({plan.progress()}){RESET} {DIM}\u2500{RESET} {plan.summary}")
        for s in plan.steps:
            if s.status == "done":
                icon = f"{GREEN}\u2705{RESET}"
                ...
                icon = f"{DIM}\u25cb{RESET}"
            label = f"{icon} {s.id}. {s.description}"
            if s.status == "running":
    ...
    def _run_questionnaire(self, questions: list) -> list[str] | None:
        """
        Show an interactive menu for each question. Each question may have pre-made
        options. User picks a number or 'c' for a custom answer. Returns list of
        "Q: ... / A: ..." strings, or None if the user cancels.
        """
        ...
        for qi, q in enumerate(questions, 1):
            # q may be a dict with "text"+"options", or a plain string
            ...
            while True:
                ...
                if options:
                    ...
                    if raw.isdigit():
                        ...
                        # If they type text directly, use it as custom answer
                        ans = raw
                        print(f"     {GREEN}\u2713 {ans}{RESET}")
                        break
                else:
    ...
    def _create_plan(self, prompt: str) -> AgentPlan | None:
        ...
        lang = self.config.lang or "en"
        if lang == "es":
            plan_prompt = f"""Eres un planificador experto. Para la siguiente tarea, crea un plan detallado paso a paso.

Tarea: {prompt}

RESPUESTA:
Opción A — Si tienes suficiente información, responde con:
{{"summary": "resumen en 1 linea", "steps": [{{"description": "descripcion", "detail": "opcional"}}]}}

Opción B — Si necesitas más detalles, responde con preguntas, cada una con OPCIONES de respuesta:
{{"type": "questions", "questions": [
  {{"text": "pregunta aqui", "options": ["Opción A", "Opción B", "Opción C"]}}
]}}
...
- Las preguntas DEBEN incluir al menos 2 opciones de respuesta predefinidas
- El campo "detail" es opcional"""
        else:
            plan_prompt = f"""You are an expert planner. For the following task, create a detailed step-by-step plan.

Task: {prompt}

RESPONSE:
Option A — If you have enough info, respond with:
{{"summary": "short summary", "steps": [{{"description": "step description", "detail": "optional"}}]}}

Option B — If you need clarification, respond with questions, each with pre-made answer OPTIONS:
{{"type": "questions", "questions": [
...
- Questions MUST include at least 2 predefined answer options
- The "detail" field is optional"""
    ...
    def _run_prompt(self, prompt: str) -> None:
        ...
        if self._plan_mode:
            if plan:
                ...
                print(f"  {DIM}Could not create plan. Executing directly.{RESET}")

        self._show_plan_ui()

        print(f"\n{_separator()}")
        print(f"  {BOLD}{CYAN}\u276f {RESET}{BOLD}{prompt}{RESET}")
        print(f"  {DIM}run #{self._total_runs}{RESET}")
        print(_separator())
        ...
        # Set up status bar for execution
        ...
        # Build in-session context from last N run transcripts
        ...
        if not self._ephemeral:
            ...
            # Inject loaded session file as prior context block
            if self._loaded_session_context:
        ...
        if plan_done:
            ...
            self._print_banner()
            self.state.plan = None  # clear plan after completion
        elif self.state.plan:
            # Plan did not complete — show what happened
            progress = ""
            if hasattr(agent, "plan_executor") and agent.plan_executor:
                progress = agent.plan_executor.progress_str()
            else:
        ...
        # Training mode: ask to save good examples
    ...
    def _maybe_save_training_example(self, prompt: str, result: AgentRunResult, active_config: Config) -> None:
        """In training mode, ask user if the run was good enough to save as a training example."""
        has_errors = any(step.result.startswith("ERROR:") for step in result.steps)
        if has_errors:
            return  # Don't save runs with errors
        ...
        # Force terminal to cooked mode — _read_input_raw uses raw/tty mode
    ...
    def _ensure_cooked_terminal(self) -> None:
        """Restore terminal to canonical (cooked) mode for input() calls."""
        try:
...
{result.answer}"""
    ...
    def _handle_agent_event(self, event: str, payload: dict) -> None:
        if event == "action_started":
            ...
            style = ACTIONS_STYLE.get(kind, {"icon": "\u2022", "color": DIM})
            icon = style["icon"]
            color = style["color"]

            if kind == "shell":
                cmd = action.get("command", "")
                self._status["current_action"] = f"shell: {cmd[:50]}"
                print(f"  {DIM}{step}.{RESET} {color}{icon} shell{RESET} {DIM}\u2014{RESET} {cmd}{RESET}")
            elif kind in {"read_file", "write_file", "append_file"}:
                path = action.get("path", "")
                self._status["current_action"] = f"{kind}: {path}"
                print(f"  {DIM}{step}.{RESET} {color}{icon} {kind}{RESET} {DIM}\u2014{RESET} {path}{RESET}")
            elif kind == "search_files":
                query = action.get("query", "")
                self._status["current_action"] = f"search: {query}"
                print(f"  {DIM}{step}.{RESET} {color}{icon} search{RESET} {DIM}\u2014{RESET} {query}{RESET}")
            elif kind == "create_skill":
                name = action.get("name", "")
                self._status["current_action"] = f"skill: {name}"
                print(f"  {DIM}{step}.{RESET} {color}{icon} skill{RESET} {DIM}\u2014{RESET} {name}{RESET}")
            elif kind == "run_skill":
                args = action.get("args", "")
                skill = action.get("skill", "")
                self._status["current_action"] = f"run: {skill}"
                print(f"  {DIM}{step}.{RESET} {color}{icon} {skill}{RESET}{DIM} {args}{RESET}")
            elif kind == "remember":
                self._status["current_action"] = "remember"
                print(f"  {DIM}{step}.{RESET} {color}{icon} remember{RESET}")
            elif kind == "skip_step":
                sid = action.get("step_id", "")
                reason = action.get("reason", "")
                self._status["current_action"] = f"skip step {sid}"
                print(f"  {DIM}{step}.{RESET} {YELLOW}\u23ed\ufe0f skip{RESET} {DIM}\u2014{RESET} step {sid}: {reason}{RESET}")
            elif kind == "final":
                self._status["current_action"] = ""
                print(f"  {DIM}{step}.{RESET} {color}{icon} {self.t('action.done')}{RESET}")
            ...
            if kind in {"shell", "run_skill"}:
                ok = result.startswith("SUCCESS:")
                label = "shell" if kind == "shell" else "skill"
                if ok:
                    print(f"    {DIM}\u2570\u2500\u2705 {label} ok{RESET}")
                else:
                    print(f"    {DIM}\u2570\u2500\u274c {label} failed{RESET}")
            elif kind == "skip_step":
                print(f"    {DIM}\u2570\u2500\u23ed\ufe0f step skipped{RESET}")
            elif kind != "final" and result:
                preview = result[:200]
    ...
    def _print_run_result(self, result: AgentRunResult) -> None:
        ...
        print(f"  {DIM}\u2500{'\u2500' * min(60, _term_width() - 4)}{RESET}")
    ...
    def _save_run(self, prompt: str, result: AgentRunResult) -> None:
        ...
        print(f"  {DIM}{self.t('answer.saved')}: {path.name}{RESET}")
    ...
    def _save_notes(self, title: str) -> None:
        ...
        print(f"{GREEN}{self.t('answer.saved_notes')}: {path}{RESET}")
