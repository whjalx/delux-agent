...
# Lightweight keyword matching that happens BEFORE calling the LLM.
# This dramatically reduces the input size for small models, improving accuracy.
...
def _heuristic_prefilter(
    ...
    for block in skill_blocks:
        ...
        # Header format: --- skill:name: summary
        header_line = block.split("\n")[0]
        keywords = set(re.findall(r"\b\w{3,}\b", header_line.lower()))
        
        # Also include the skill name as a mandatory keyword
        if skill_name:
    ...
    # --- Memory Filtering ---
    # Keep lines that have words in common with the prompt (simple but effective)
...
class Contextualizer:
    ...
    def _ensure_dataset_sync(self):
        ...
        # Limitar estrictamente a un máximo de 700 para mantener el balance entre inteligencia y VRAM
        ...
        if current_size != target_size:
            ...
            from .setup import _generate_training_dataset
    ...
    def contextualize(
        ...
        # Si es un saludo o cortesía, respondemos de forma humana y saltamos la IA técnica
        ...
        # Primero inyectamos lo aprendido por el agente (Prioridad Máxima)
        if learned_path.exists():
            try:
                with open(learned_path, "r", encoding="utf-8") as f:
                    learned_cases = json.load(f)[:50] # Solo los 50 éxitos más recientes para no saturar
                    for case in learned_cases:
                        expert_examples_text += f"\n--- self-learned expert ---\n{case['user']}\nSTRATEGY: {case['assistant']}\n"
            except:
                pass

        # Luego añadimos la librería estándar (si queda espacio)
        training_path = self.config.root / "training" / "training_examples.md"
        if training_path.exists():
        ...
        # Si el prompt es muy corto (saludo), lo procesamos igual para asegurar humanidad
        is_short_chat = len(user_prompt.split()) < 3
        
        if pre_tokens < 2 and not is_short_chat:
            # Context is tiny — skip LLM, just pass through
            return ContextualizedPrompt(
                original_tokens=original_tokens,
                optimized_tokens=pre_tokens,
                savings_pct=projected_savings,
                prompt=user_prompt,
        ...
        # Priority 1: Training Dataset (Static for caching)
        training_path = self.config.root / "training" / "training_examples.md"
        if training_path.exists():
            try:
        ...
        # Priority 2: The actual task and current status
        if plan_context.strip():
            # If there's a plan, we MUST emphasize it after the static knowledge
        ...
        try:
            ...
            # Instead of blocking, we remind the agent to be smart about resources
            ...
            # ── Stage 6: Graceful fallback with pre-filtered context ──────────
            # Even if the LLM fails, the pre-filter already saved tokens
            fallback_prompt = user_prompt
            if prefilter_changes:
                # Use pre-filtered skills at least
                fallback_prompt = user_prompt
            return ContextualizedPrompt(
                original_tokens=original_tokens,
